...
esp_err_t http_client_set_header_cb(esp_http_client_handle_t http_client) {
  return esp_http_client_set_header(http_client, "User-Agent", "Yac-ESP32-" YAC_VERSION);
}
...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  ...
  esp_err = esp_http_client_set_header(client_handle, "User-Agent", "Yac-ESP32-" YAC_VERSION);
  if (esp_err != ESP_OK) {
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == YAC_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = YAC_VERSION;
